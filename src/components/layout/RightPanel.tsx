    ...
    <div style={{ padding: 12, overflowY: 'auto', height: '100%' }}>
      ...
      {sc.type === 'text' && (
        <>
          ...
          {selectField('Type', 'animationType', sc.animationType || 'none', [
          ...
          {sc.animationType && sc.animationType !== 'none' && (
            <>
              {numField('Speed', 'animationSpeed', sc.animationSpeed ?? 1, { min: 0.1, max: 5, step: 0.1 })}
              {(sc.animationType === 'bounce' || sc.animationType === 'wave') &&
                numField('Amplitude', 'animationAmplitude', sc.animationAmplitude ?? 10, { min: 1, max: 50 })}
              {numField('Char Delay', 'animationDelay', sc.animationDelay ?? 50, { min: 0, max: 500, step: 10, unit: 'ms' })}
            </>
          )}
        </>
      )}
